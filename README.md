...
自動將音樂拆分為人聲、鼓、貝斯、吉他、鋼琴、其他六條音軌，並提供混音播放、音量調整、MP3 下載、BPM/調性偵測功能。
...
| S | 獨奏（Solo）—整體播放時只聽此軌 |
| 音量滑桿 | 0–150%，即時調整 |
| ⬇ MP3 | 下載此音軌（套用目前音量與速度，MP3 320kbps） |
...
| ▶ 整體播放 | 所有音軌同步混音播放 |
| 進度條 | 可拖曳跳轉播放位置 |
| 速度 | 分源後以 librosa beat_track 偵測 BPM（精度較高），失敗時退回自製 Fourier tempogram；可手動調整 |
| 調性 | 分源後自動偵測調性，下拉選單以偵測調性為中心，往上 +8 半音（升記號）、往下 −8 半音（降記號）；切換後立即以 RubberBand 高品質移調 |
| 整體音量 | 疊加在各軌音量之上 |
| ⬇ 下載混音 MP3 320k | 依目前音量/靜音/速度/調性狀態混音後下載 |
...
├── core/
│   ├── separator_process.py     # 分源進程管理器（持久化子進程，支援即時取消）
│   ├── separator_worker.py      # 分源 Worker（在子進程中執行，可被 kill 停止）
│   ├── separator_thread.py      # 舊版執行緒實作（備用）
│   ├── player.py                # 多軌同步播放引擎
│   ├── mixer.py                 # 混音邏輯
│   ├── exporter.py              # MP3 匯出
│   ├── pitch.py                 # 串流相位聲碼器（即時移調預覽）
│   ├── bpm.py                   # BPM 偵測（librosa + fallback）
│   └── key.py                   # 調性偵測
└── ui/
    ├── main_window.py           # 主視窗
    ...
    ├── progress_dialog.py       # 分源進度（含取消按鈕）
    └── styles.qss               # 淺色主題樣式（Indigo 主色調）
```

> **分源架構說明：** 分源使用獨立子進程執行（而非執行緒），因此按下取消後 CPU 可立即釋放。子進程在應用程式生命週期內持久存活，後續分源不需重新啟動，速度與原本相同。
...
A：請用滑鼠右鍵點 `setup.bat` → 「以系統管理員身分執行」，部分系統安裝需要管理員權限。
...
**Q：按下取消後 CPU 還是很高？**
A：不應發生此情況，取消會直接 kill 分源子進程，CPU 應立即下降。若仍持續，請重新啟動程式。
